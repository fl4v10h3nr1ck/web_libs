	...
	function pesquisaRestaurada(id_tab){
		...
		$.post(	
			...
			function(retorno){
				...
				if($("#func_pos_pesquisar_"+id_tab).length && $("#func_pos_pesquisar_"+id_tab).val().length>0)
					window[$("#func_pos_pesquisar_"+id_tab).val().replace("(", "").replace(")", "")]();

				if($("#func_salva_estado_"+id_tab).length && $("#func_salva_estado_"+id_tab).val().length>0)
					window[$("#func_salva_estado_"+id_tab).val()]();
			}
		);
	}
