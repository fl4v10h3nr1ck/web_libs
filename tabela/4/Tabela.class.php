...
private $func_salva_estado;
	...
	public function setFuncaoSalvaEstado($funcao){
		
		$this->func_salva_estado= $funcao;	
	}
...
/************************************************************************/	
	...
	public function getTabela($nome_objeto, $nome_campo_id, $executar_pesquisa_inicial = true){
		...
		<input type='hidden' id='func_salva_estado_".		$id_tab."' value='".$this->func_salva_estado."'>";
	...
	private function getQueryTermos($objeto, $metadados){
		...
		$como_data = $pos_local[1]>0?true:false;
			...
			foreach($metadados['props'] as $meta){
				...
				if($meta['posicao'] == $pos_local[0]){
					...
					$pesquisar_null_ou_vazio = $meta['pesquisar_null_ou_vazio'];
		...
		if(strcmp($tipo, "data")==0 && $como_data){
			...
			$query_termo .= " (";
				...
				if(strlen($value)==0 && $pesquisar_null_ou_vazio)
					$query_termo .= "(".$local."='' or ".$local." IS NULL)";
				else
		...
		return $query_termo." or ".($pesquisar_null_ou_vazio?1:0);	
	}
	...
	$metadados['props'] = array();
		...
		$reflexao_prop = new ReflectionAnnotatedProperty($objeto, $prop->getName());
			...
			$metadados['props'][] = array(
								 ...
								 "pesquisar_null_ou_vazio"=>($reflexao_prop->getAnnotation('AnotColuna')->pesquisar_null_ou_vazio==null || 
																	$reflexao_prop->getAnnotation('AnotColuna')->pesquisar_null_ou_vazio===false?false:true));
			}
		}
